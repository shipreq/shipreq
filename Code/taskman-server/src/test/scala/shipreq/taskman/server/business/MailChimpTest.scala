...
class MailChimpTest extends Specification {
  ...
  def testParse[A](f: JValue => ArticulateError \/ A, txt: String): A =
    parseIntoJson(txt).flatMap(f).fold(throw _, identity)

  "Total API failures" >> {

    "parseHttpErrorJson" in {
      testParse(ApiFailure.Total.parseHttpErrorJson,
  ...
  "lists/list" >> {
    "ok" in {
      testParse(parseResponse(GetListId("")),
    ...
    "no match" in {
      testParse(parseResponse(GetListId("")), """{"total":0,"data":[],"errors":[]}""") must beNone
    }
  }
  ...
  "lists/batch-subscribe" >> {
    ...
    "error" in {
      parseIntoJson("""{"add_count":0,"adds":[],"update_count":0,"updates":[],"error_count":1,"errors":[{"code":250,"error":"ACCT must be provided - Value must be one of: Never, Active (not Activ)","email":{"email":"great@yay.com"}}]}""")
        .flatMap(catchPartialFailures)
        .flatMap(parseResponse(BatchSubscribe(null, null)))
        .must(beAnError)
    }
  }
  ...
  "lists/subscribe" >> {
    "error parsing" in {
      val f = testParse(ApiFailure.Total.parseHttpErrorJson,
        """{"status":"error","code":214,"name":"List_AlreadySubscribed","error":"tmp-mailchimp-app@shipreq.com is already subscribed to list Master. Click here to update your profile."}""")

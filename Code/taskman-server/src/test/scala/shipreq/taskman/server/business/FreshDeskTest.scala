...
class FreshDeskTest extends Specification {
  ...
  "Get groups" should {
    ...
    "find the id of a group by name" in {
      respOk flatMap parseRespGetGroup("QA") must beNonErrorOf(Group(1000123402, "QA"))
    }

    "fail if no group returned with given name" in {
      respOk flatMap parseRespGetGroup("WHAT") must beAnError
    }
  }
  ...
  "Create new ticket" should {
    ...
    "Parse success" in {
      respOk flatMap parseCreateTicketResponse must beNonErrorOf(TicketId(5))
    }
  }
}

...
class TextFieldText extends FunSpec with ShouldMatchers with MockitoSugar {
  ...
  describe("load()") {
    ...
    it("should clear value when no field value exists") {
      val tr = new TextField(ATextFieldDef, mock[UCEditorState], Key_1)
    ...
    it("should change its value to the loaded field value") {
      val tr1 = new TextField(ATextFieldDef, mock[UCEditorState], Key_1)
      val tr2 = new TextField(ATextFieldDef, mock[UCEditorState], Key_2)

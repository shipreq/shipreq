...
//  }
  ...
  implicit class ASlkjdashfglkjhsdg(private val self: KeyboardEventData) extends AnyVal {
    def alt   = self.copy(altKey   = true)
    def ctrl  = self.copy(ctrlKey  = true)
    def meta  = self.copy(metaKey  = true)
    def shift = self.copy(shiftKey = true)

...
final case class CallbackH[B, M[_], S, C](event: CallbackEvent[B], st: ReactST[M, S, Unit], data: C) {
  ...
  def mapB[Q](f: B => Q): CallbackH[Q, M, S, C] = copy(event = event map f)
...
//case class EditorCallbacks[M[_]: Bind: Applicative, S, I, C, D](t: (CallbackEvent[I], C, ReactST[M, S, Unit]) => D) {
//  final type ST = ReactST[M, S, Unit]
//  @inline final def apply(i: CallbackEvent[I], c: C): D = t(i, c)

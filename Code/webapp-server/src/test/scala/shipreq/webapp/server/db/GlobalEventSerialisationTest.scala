...
object GlobalEventSerialisationTest extends TestSuite {
  ...
  private def propTestRoundTrip(gen: Gen[GlobalEvent])(implicit l: Line): Unit =
...
//    Some(a)
  ...
  override def tests = Tests {
    ...
    "UserGroupCreate" - propTestRoundTrip(
      Gen.lift5(
        RandomData.userId,
        RandomData.userGroups.id,
        RandomData.userGroups.name,
        RandomData.userGroups.handle,
        RandomData.userGroups.arelSetWithIds,
      )(UserGroupCreate.apply)
    )

    "UserGroupUpdate" - propTestRoundTrip(
      Gen.lift5(
        RandomData.userId,
        RandomData.userGroups.id,
        RandomData.userGroups.name.option,
        RandomData.userGroups.handle.option,
        RandomData.userGroups.arelSetDiffWithIds,
      )(UserGroupUpdate.apply)
    )
